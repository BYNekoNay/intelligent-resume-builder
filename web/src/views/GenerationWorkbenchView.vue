...
const materialsByDecision = computed(() => {
  const must: CareerMaterial[] = []
  const exclude: CareerMaterial[] = []
  const auto: CareerMaterial[] = []
...
onMounted(async () => {
  await Promise.all([materialStore.load(), jdStore.load()])
...
async function startGeneration() {
  ...
  try {
    ...
    if (!consent || consent.eventType !== 'GRANTED') {
      router.push('/ai-consent?redirect=/generate')
      return
    }
    ...
    const res = await generateForJob(payload as any, idempotencyKey)
    const task = res.data.data
    taskId.value = task.id
    taskStore.remember(task.id)

    // Navigate to confirmation page
    router.push(`/generate/confirm?taskId=${task.id}`)
  } catch (e: any) {
