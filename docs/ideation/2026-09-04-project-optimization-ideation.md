...
> ## ⚠ 阅读指引与时效性声明（2026-09-25 补充）
>
> 本文是 **2026-09-04 那一天**针对分支 `codex/resume-loop-enhancements` 的一次点状审计，当天连续追加了 46 轮，
> 因此**同一份文档内存在前后不一致**：顶部列的是"当时仍存在"，底部记的是"后来已修复"。
>
> **结构说明（读之前先看这一节）**
>
> | 章节 | 性质 |
> | --- | --- |
> | §「已闭环的历史问题」 | O-01~O-14，写进本文时即已闭环 |
> | §「当前仍存在的错误或风险」第 1~38 项 | **2026-09-04 当时的快照**，不代表今天 |
> | §第 40~46 轮「审查问题修复」 | 后续轮次的修复记录，**其中部分修的就是上面那 38 项** |
>
> **已知的直接冲突（顶部标"仍存在"、底部标"已修复"）—— 这三项不要再按现存风险处理**：
> **#10**（AI 任务数据类别映射不一致）、**#34**（账号删除后的异步任务与 PDF worker fencing）、
> **#35**（AI 职业资料快照 PII 脱敏）。
> 另经代码复核，**#6**（账号删除后 access token 仍有效）与 **#18**（提供者路由无失败切换）同样已修复。
>
> **单一权威来源**：当前状态以 `PROJECT_CONTEXT.md` 与磁盘代码为准；仍未闭环的事项以
> `docs/decisions/OPEN-DECISIONS.md` 为准。本文只作**审计思路与候选清单**使用，不作现状依据。
>
> **可复用的教训**：三周前的缺口清单会随修复而失效。引用任何"现存问题清单"前**必须逐条核实当前状态**，
> 否则会重复修一个不存在的问题、或漏掉真问题。
