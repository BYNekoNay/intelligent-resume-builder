...
> **修复状态（2026-09-26，提交 d10ee13）**：其中**无需产品决策且行为风险可控的 20 项已修复**
> （错误码对齐、限流器枚举化、注释漂移 ×4、yml 显式化、归档文案区分、注册枚举统一、
> consent 精确匹配、logout 取证保留、restore 死代码、setCurrentVersion DTO、抽取器健壮性测试、
> 空词典契约注释、前端假 taskType/双击防护/轮询上限、careermaterial 测试缺口 4 项）；
> **其余各项因需产品口径或存在行为风险而暂缓**，逐项保留于下表（含理由标注于本轮提交 d10ee13）。
> 全量测试 695 个 0 失败；web build 全绿。
