...
| `application` | 投递 CRUD、乐观锁与有限状态迁移 |
| `communication` | 基于简历/JD 事实生成并持久化人工确认草稿，绝不自动发送 |
| `interview` | 有限轮次面试、确定性反馈与报告、答案资产 CRUD 和筛选 |
| `imports` | TXT/PDF/DOCX 文本抽取与基础字段规范化，原文件不落盘 |
...
- AI 的 `retryCount` 只在 worker 实际领取执行时增加；手动重试必须重新通过用户归属、授权范围、当日配额和最大次数检查。
- 内联润色 `INLINE_OPTIMIZE` 仅针对可叙述字段返回候选文本，用户点击“采纳并写回”才修改编辑器；候选与原文相同会被过滤，AI 不得自动补充事实、数字、链接、机构或经历。

### AI 与隐私边界

- AI 授权版本当前为 `v1.2.0`，任务执行时校验任务范围和数据类别，撤销授权后不得继续执行。
