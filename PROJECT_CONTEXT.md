...
- 简历编辑器是 `ResumeEditorView.vue`：使用当前版本加载内容，保存时创建版本；支持模块内条目拖拽及工作/技能/项目/教育/证书/语言模块排序，预览同步更新。
- AI 接口大多是异步任务：创建接口返回 `202 + taskId`，前端必须轮询 `GET /api/ai/tasks/{id}` 至 `SUCCESS` 或失败，不能把任务创建响应当作最终结果。
- 内联润色 `INLINE_OPTIMIZE` 返回候选文本，用户点击“采纳并写回”才修改编辑器；候选与原文相同会被过滤。
